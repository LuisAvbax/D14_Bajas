...
@Repository
public class ReporteDetalladoAjusteTipoSercotelDaoImpl extends AbstractDaoImpl<v_bajas_nvo3, Integer> implements ReporteDetalladoAjusteTipoSercotelDao, Serializable{
	...
	final static Logger logger = LoggerFactory.getLogger(ReporteDetalladoAjusteTipoSercotelDaoImpl.class);
	@Autowired
	...
	public ReporteDetalladoAjusteTipoSercotelDaoImpl(){
...
public ReporteDetalladoPorTipoBean consultaReporteDetalladoTipo(ReporteDetalladoPorTipoBean datos) throws CfeException, SQLException {
	...
	try{
		...
		conn = dataSourceSercotel.getConnection();
			...
			StringBuffer selectSQL = new StringBuffer("select 1 as orden, 'ACTIVOS CON VALOR -1.00' as activo,region, clase, dia, mes,   "
					+ " anio, texto_baja, denom,num_activo, sub, per_baja, "
					+ " adq_baja, adq_ac_baja, ejercicio_baja, depr_tot, costo_h, TRUNC(MAX(inpc_mp),4) inpc_mp, TRUNC(MAX(inpc),4) inpc, fac_act,");
					selectSQL.append(" costo_act, depre_anual_act ");
					...
					selectSQL.append("group by region, clase, dia, mes, anio, texto_baja, denom,num_activo, sub, per_baja,"
							+ " adq_baja, adq_ac_baja, ejercicio_baja, depr_tot,costo_h, fac_act, costo_act, depre_anual_act ");
					
					selectSQL.append("union ");
					selectSQL.append("select 2 as orden, 'ACTIVOS CON VALOR -0.01' as activo,   "
							+ " region, clase, dia, mes, anio, texto_baja, "
							+ "	denom,num_activo, sub, per_baja,adq_baja, ");
					selectSQL.append(" adq_ac_baja, ejercicio_baja, depr_tot, "
							+ "	costo_h, TRUNC(MAX(inpc_mp),4) inpc_mp, TRUNC(MAX(inpc),4) inpc, fac_act, "
							+ " costo_act, depre_anual_act ");
					selectSQL.append("from v_bajas_nvo4 ");
					selectSQL.append(" where d_inv= '");
					selectSQL.append( datos.getdInv() +"' ");
					selectSQL.append( " AND area_val='");
					...
					selectSQL.append("union ");
					selectSQL.append("select 3 as orden, 'ACTIVOS 100% DEPRECIADOS', "
							+ "region, clase, dia, mes, anio, texto_baja, "
							+ "denom,num_activo, sub, per_baja,0.00 adq_baja, ");
					selectSQL.append("0.00 adq_ac_baja, 0.00 ejercicio_baja, 0.00 depr_tot, "
							+ "0.00 costo_h, TRUNC(MAX(inpc_mp),4) inpc_mp, TRUNC(MAX(inpc),4) inpc, fac_act,"
							+ " costo_act, depre_anual_act");
					selectSQL.append(" from v_bajas_nvo4 ");
					selectSQL.append(" where d_inv= '");
					selectSQL.append( datos.getdInv() +"' ");
					...
					selectSQL.append("order by 1,clase, region, texto_baja ");
			...
			while (rs.next()) {
				...
				reporte.setAnio(rs.getInt("anio"));
				 try {
					 dateInString = reporte.getDia() + "-" + reporte.getMes() + "-" + reporte.getAnio();
				...
				datos.getTotReporteGeneral().setAdq_ac_baja(datos.getTotReporteGeneral().getAdq_ac_baja().add(rs.getBigDecimal("adq_ac_baja")!=null?rs.getBigDecimal("adq_ac_baja"):BigDecimal.ZERO));
				datos.getTotReporteGeneral().setEjercicio_baja(datos.getTotReporteGeneral().getEjercicio_baja().add(rs.getBigDecimal("ejercicio_baja")!=null?rs.getBigDecimal("ejercicio_baja"):BigDecimal.ZERO));
				datos.getTotReporteGeneral().setDepr_tot(datos.getTotReporteGeneral().getDepr_tot().add(rs.getBigDecimal("depr_tot")!=null?rs.getBigDecimal("depr_tot"):BigDecimal.ZERO));
				datos.getTotReporteGeneral().setCosto_h(datos.getTotReporteGeneral().getCosto_h().add(rs.getBigDecimal("costo_h")!=null?rs.getBigDecimal("costo_h"):BigDecimal.ZERO));
...
public List<TGTipoAjusteBean> generaTree(List<BajasDosBean> listDatos){
	...
	for(BajasDosBean dta: listDatos){
		if(!tipoList.contains(new TGTipoAjusteBean(dta.getCvetipoAjuste()))){
			...
			totalTipo.setAdq_ac_baja(BigDecimal.ZERO);
			totalTipo.setEjercicio_baja(BigDecimal.ZERO);
			totalTipo.setDepr_tot(BigDecimal.ZERO);
			totalTipo.setCosto_h(BigDecimal.ZERO);
			...
			totalTipo.setAdq_ac_baja(totalTipo.getAdq_ac_baja().add(dta.getAdq_ac_baja()!=null?dta.getAdq_ac_baja():BigDecimal.ZERO));
			totalTipo.setEjercicio_baja(totalTipo.getEjercicio_baja().add(dta.getEjercicio_baja()!=null?dta.getEjercicio_baja():BigDecimal.ZERO));
			totalTipo.setDepr_tot(totalTipo.getDepr_tot().add(dta.getDepr_tot()!=null?dta.getDepr_tot():BigDecimal.ZERO));
			totalTipo.setCosto_h(totalTipo.getCosto_h().add(dta.getCosto_h()!=null?dta.getCosto_h():BigDecimal.ZERO));
