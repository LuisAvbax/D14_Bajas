...
@Repository
public class TotalGlobalSercotelDaoImpl extends AbstractDaoImpl<v_bajas_nvo3, Integer> implements TotalGlobalSercotelDao, Serializable{
	 ...
	 * 
	 */
	final static Logger logger = LoggerFactory.getLogger(TotalGlobalSercotelDaoImpl.class);
	@Autowired
	...
	public TotalGlobalSercotelDaoImpl(){
...
public TotalGlobalRegBean consultaTotalGlobalRegion(TotalGlobalRegBean datos) throws CfeException, SQLException {
	...
	try{
		...
		String[] values = null;
		List<String> possibleValue = null;
		StringBuilder builder = null;
	
			StringBuilder selectSQL = new StringBuilder("select region,region_dsc,clase||'-'||clase_dsc clase_dsc,texto_baja,  "
					+ " sum(nvl(adq_baja,0)) adq_baja, sum(nvl(adq_ac_baja,0)) adq_ac_baja, "
					+ "sum(nvl(ejercicio_baja,0)) ejercicio_baja, sum(nvl(depr_tot,0)) depr_tot, "
					+"sum(nvl(costo_h,0)) costo_h, sum(nvl(costo_act,0)) costo_act, " 
					+"sum(nvl(depre_anual_act,0)) depre_anual_act "
					+"from v_bajas_nvo3 ");
			selectSQL.append(" where d_inv= ? ");
			selectSQL.append( " AND area_val= ? ");
			values = datos.getMesesConsulta().split(",");
			possibleValue = Arrays.asList(values);
			builder = utileriasCfeDao.buildInClause(possibleValue);
			selectSQL.append( " AND per_baja in ("+ builder.deleteCharAt( builder.length() -1 ).toString() +")");
			if (datos.getRegion() != null && datos.getRegion().size() > 0)
			{
				values = filterRegion.split(",");
				possibleValue = Arrays.asList(values);
				builder = utileriasCfeDao.buildInClause(possibleValue);
				selectSQL.append( " AND region IN ("+ builder.deleteCharAt( builder.length() -1 ).toString() +")");
			}
			if(datos.getClase() != null && datos.getClase().size() > 0) 
			{
				values = filterClase.split(",");
				possibleValue = Arrays.asList(values);
				builder = utileriasCfeDao.buildInClause(possibleValue);
				selectSQL.append( " AND clase IN ("+ builder.deleteCharAt( builder.length() -1 ).toString() +")");
			}
			if("TODOS".equalsIgnoreCase(datos.getTexto()) && datos.getTxtDesc() != null && datos.getTxtDesc().size()>0){
				if(filterTxt != null){
					values = filterTxt.split(",");
					possibleValue = Arrays.asList(values);
					builder = utileriasCfeDao.buildInClause(possibleValue);
					selectSQL.append( " AND texto_baja in ("+ builder.deleteCharAt( builder.length() -1 ).toString() +")");
				}else{
					selectSQL.append( " AND texto_baja in ("+ filterTxt +")");
				}
			...
			selectSQL.append( " group by region,region_dsc,3, texto_baja  ");
			selectSQL.append( " order by region,region_dsc,3 asc, texto_baja  ");
			logger.info(">>>Query totalGLobal: " + selectSQL);
			preparedStatement = conn.prepareStatement(selectSQL.toString());
			preparedStatement.setInt(++countParam, datos.getdInv());
			preparedStatement.setInt(++countParam, datos.getAreaVal());
			
			values = datos.getMesesConsulta().split(",");
			possibleValue = Arrays.asList(values);
			for(String val:possibleValue){
				preparedStatement.setString(++countParam, val);
			}
			if (datos.getRegion() != null && datos.getRegion().size() > 0)
			{
				for(String val:datos.getRegion()){
					preparedStatement.setString(++countParam, val);
				}
			}
			if(datos.getClase() != null && datos.getClase().size() > 0) 
			{
				for(String valCse:datos.getClase()){
					preparedStatement.setString(++countParam, valCse);
				}
				
			}
			if("TODOS".equalsIgnoreCase(datos.getTexto()) && datos.getTxtDesc() != null && datos.getTxtDesc().size()>0){
				for(String valTxt:datos.getTxtDesc()){
					preparedStatement.setString(++countParam, valTxt);
				}
			}else if(("FISCALES".equalsIgnoreCase(datos.getTexto()) || "NO FISCALES".equalsIgnoreCase(datos.getTexto())) && (datos.getTxtDesc() != null && datos.getTxtDesc().size() > 0)){
				for(String valTxt:datos.getTxtDesc()){
					preparedStatement.setString(++countParam, valTxt);
				...
				for(String valTxt:datos.getTxtsDesc()){
					preparedStatement.setString(++countParam, valTxt);
				}
			}
			...
			datos.setListTotalGlobalReg(listReporte);
...
public TotalGlobalRegBean consultaTGlobalRegNetos(TotalGlobalRegBean datos) throws CfeException, SQLException {
	...
	try{
		...
		conn = dataSourceSercotel.getConnection();
			...
			selectSQL.append(" sum(nvl(adq_baja,0))*-1 adq_baja, sum(nvl(adq_ac_baja,0))*-1 adq_ac_baja, sum(nvl(ejercicio_baja,0))*-1 as ejercicio_baja, sum(nvl(depr_tot,0))*-1 depr_tot, ");
			selectSQL.append(" sum(nvl(costo_h,0))*-1 costo_h, sum(NVL(costo_act,0))*-1 as costo_act, sum(NVL(depre_anual_act,0))*-1 as depre_anual_act ");
			...
			selectSQL.append( " union  ");
			selectSQL.append("select region,region_dsc,clase||'-'||clase_dsc clase_dsc,texto_baja,  ");
			selectSQL.append(" 0.0 as adq_baja, 0.0 as adq_ac_baja, 0.0 as ejercicio_baja, 0.0 as depr_tot, ");
			selectSQL.append(" 0.0 as costo_h, sum(NVL(costo_act,0))*-1 as costo_act, sum(NVL(depre_anual_act,0))*-1 as depre_anual_act ");
			selectSQL.append(" from v_bajas_nvo4 ");
			selectSQL.append(" where d_inv= '");
			selectSQL.append( datos.getdInv() +"'");
			selectSQL.append( " AND area_val='");
...
public TotalGlobalClaBean consultaTotalGlobalClase(TotalGlobalClaBean datos) throws CfeException, SQLException {
	...
	try{
		...
		StringBuffer selectSQL = new StringBuffer("select clase||'-'||clase_dsc clase_dsc, region, region_dsc, texto_baja, "
					+ "sum(adq_baja) adq_baja, sum(adq_ac_baja) adq_ac_baja, sum(ejercicio_baja) ejercicio_baja, "
			...
			selectSQL.append( " group by 1,region,region_dsc,texto_baja");
			selectSQL.append( " order by 1 asc,region asc,texto_baja   ");
	...
	try{
		...
		conn = dataSourceSercotel.getConnection();
			...
			StringBuffer selectSQL = new StringBuffer("select 1 as orden, 'IMPROCEDENTES CON VALOR -1.00' as activo,   sum(NVL(adq_baja,0)) as adq_baja, sum(NVL(adq_ac_baja,0)) as adq_ac_baja, sum(NVL(ejercicio_baja,0)) as ejercicio_baja,  ");
					...
					selectSQL.append("sum(0.00) as depr_tot, sum(0.00) as costo_h, sum(NVL(costo_act,0)) as costo_act, sum(NVL(depre_anual_act,0)) as depre_anual_act ");
					selectSQL.append("from v_bajas_nvo4 ");
					selectSQL.append(" where d_inv= '");
					selectSQL.append( datos.getdInv() +"' ");
					selectSQL.append( " AND area_val='");
