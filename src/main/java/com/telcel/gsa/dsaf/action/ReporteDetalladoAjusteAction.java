 ...
 *
 */
@Service("repDetalladoAjustAct")
@ViewScoped
public class ReporteDetalladoAjusteAction implements Serializable{

	private static final long serialVersionUID = 1357848192092043408L;

	final static Logger logger = LoggerFactory.getLogger(ReporteDetalladoAjusteAction.class);
	...
	@Autowired
	private ReporteDetalladoAjusteService reporteDetalladoAjusteService;
	...
	public void initFlow(RequestContext ctx){
		...
		ctx.getFlowScope().put("repoDetalleAjust", reporteDetallado);
	}
	
	public void computeYearVal(RequestContext ctx){
		ReporteDetalladoPorTipoBean reporteDetallado = (ReporteDetalladoPorTipoBean)ctx.getFlowScope().get("repoDetalleAjust");
	...
	public void consulta(RequestContext ctx) throws CfeException, SQLException{
		...
		reporteDetallado = reporteDetalladoAjusteService.datosWhere(reporteDetallado);
		...
		reporteDetallado.setAnioRepCorto(an);
			...
			reporteDetallado = reporteDetalladoAjusteService.consultaReporteDetalladoAjuste(reporteDetallado, sessionScopeUser);
	...
	public void descarga(RequestContext ctx){
		...
		httpServletResponse.addHeader(Constants.REP_RESUMENR_CONTENTDISP, Constants.REP_RES_DET_AJ_TPO_FILENAME 
		...
		try {
			HSSFWorkbook workbook = reporteDetalladoAjusteService.generaDocumento(reporteDetallado);
	...
	public void descargaPDF(RequestContext ctx) throws ServletException, IOException{
		ReporteDetalladoPorTipoBean datos = (ReporteDetalladoPorTipoBean)ctx.getFlowScope().get("repoDetalleAjust");
		...
		jParams.put("titulo2", Constants.TIT_REP_DETALLADO_AJUSTE);
		...
		fullPathRutaArch.append("repotDetaAjusteP1.jasper");//nombre del archivo
		...
		try {
			...
			(HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
			 ...
			 httpServletResponse.setHeader("Content-disposition", Constants.REP_RES_DETALLADO_AJUSTE_FILENAME + Constants.REP_PDF_EXT);
